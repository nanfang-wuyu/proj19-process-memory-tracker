// idea from [1] Bornander, F. (2012, June 6). C++ Memory Leak Finder. CodeProject.
// https://www.codeproject.com/articles/393957/cplusplus-memory-leak-finder.
...
/* Show the information of threads having leak. */
void show_thread(){
...
/* Show the information of allocations having leak (not been released). */
...
/* Look up pointers to the actual implementation. */
...
/* Executed typically at program exit, which iterates through the leaks and output the leak information. */
void compile_allocation()
{
    isExternalSource = false;
    ifstream file("src/order/mode.txt");
    string mode;
    file >> mode;
    file.close();
    ...
    // cout << "" << endl;
    show_allocation();
    save_allocation(allocation_list, "src/output/allocation.csv");
    save_thread(thread_list, "src/output/thread.csv");
    // cout << mode <<endl;
